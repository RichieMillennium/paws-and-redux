...
export interface IHeaderGroup extends Classy {
  floatRight?: boolean;
}
 ...
 * @constructor
 */
export const HeaderGroup: React.FunctionComponent<React.PropsWithChildren<IHeaderGroup>> = ({
  floatRight = false,
  className,
  children,
  ...props
  ...
  return (
    <ul
      {...props}
      className={`${className} flex flex-col mt-4 -mx-4 pt-4 border-t md:flex-row md:items-center md:mx-0 md:mt-0 md:border-0 md:pt-0 ${marginClasses}`}
    >
      {children}
    </ul>
  );
